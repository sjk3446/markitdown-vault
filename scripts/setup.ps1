...
& $PythonExe -m pip install "markitdown[all]" markitdown-ocr "pymupdf>=1.24,<1.27" openai imageio-ffmpeg fastapi "uvicorn[standard]" python-multipart keyring
if ($LASTEXITCODE -ne 0) {
    ...
    & $PythonExe -m pip install `
        ...
        markitdown-ocr "pymupdf>=1.24,<1.27" openai youtube-transcript-api imageio-ffmpeg `
